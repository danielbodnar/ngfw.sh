...
pub async fn change_password(mut req: Request, ctx: RouteContext<()>) -> Result<Response> {
    let auth = authenticate(&req, &ctx.env).await.map_err(|e| Error::from(e.error.message))?;
    let change: ChangePasswordRequest = req.json().await.map_err(|_| Error::from("Invalid JSON"))?;
...
pub async fn revoke_session(req: Request, ctx: RouteContext<()>) -> Result<Response> {
    let auth = authenticate(&req, &ctx.env).await.map_err(|e| Error::from(e.error.message))?;
    let session_id = ctx.param("id").ok_or_else(|| Error::from("Missing session ID"))?;
...
pub async fn remove_payment_method(req: Request, ctx: RouteContext<()>) -> Result<Response> {
    let auth = authenticate(&req, &ctx.env).await.map_err(|e| Error::from(e.error.message))?;
    let method_id = ctx.param("id").ok_or_else(|| Error::from("Missing payment method ID"))?;
...
pub async fn download_invoice(req: Request, ctx: RouteContext<()>) -> Result<Response> {
    let auth = authenticate(&req, &ctx.env).await.map_err(|e| Error::from(e.error.message))?;
    let invoice_id = ctx.param("id").ok_or_else(|| Error::from("Missing invoice ID"))?;
