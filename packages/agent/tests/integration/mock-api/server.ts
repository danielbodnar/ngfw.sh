...
const server = Bun.serve({
  ...
  websocket: {
    open(ws) {
      console.log(`[WebSocket] Connection opened from device: ${ws.data.deviceId}`);
    },
        ...
        switch (msg.type) {
          case "AUTH": {
            ...
            if (device_id === TEST_DEVICE_ID && api_key === TEST_API_KEY) {
              ...
              console.log(`[${timestamp}] AUTH_OK sent for device ${device_id}`);
            } else {
              const response = {
                id: randomUUID(),
                type: "AUTH_ERROR",
    ...
    close(ws) {
      console.log(`[WebSocket] Connection closed for device: ${ws.data.deviceId}`);
      authenticated = false;
    },
  },
});
...
console.log(`Test credentials: device_id=${TEST_DEVICE_ID}, api_key=${TEST_API_KEY}`);
