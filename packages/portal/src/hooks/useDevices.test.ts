...
import { useDevices, useDeviceStatus, useRegisterDevice } from './useDevices';
import { mockGetToken } from '../test/mocks/clerk';
import type { Device, DeviceStatus, DeviceRegistrationResponse } from '../api';
...
describe('useDeviceStatus', () => {
  ...
  it('should update status on each poll', async () => {
    const status1 = { ...mockStatus, metrics: { ...mockStatus.metrics!, cpu: 23 } };
    const status2 = { ...mockStatus, metrics: { ...mockStatus.metrics!, cpu: 45 } };
  ...
  it('should switch polling to new device when deviceId changes', async () => {
    const status1 = { ...mockStatus, device: { ...mockDevice, id: 'device-1' } };
    const status2 = { ...mockStatus, device: { ...mockDevice, id: 'device-2' } };
...
describe('useRegisterDevice', () => {
  ...
  it('should set loading state during registration', async () => {
    ...
    const promise = result.current.register({ name: 'New Device' });
  ...
  it('should register multiple devices sequentially', async () => {
    ...
    const device1 = await result.current.register({ name: 'Device 1' });
    expect(device1.id).toBe('device-1');

    const device2 = await result.current.register({ name: 'Device 2' });
