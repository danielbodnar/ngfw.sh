...
const generateTrafficLogs = () => {
	...
	return Array.from({ length: 100 }, (_, i) => ({
		...
		src:
			i % 7 === 0
				? dstIps[Math.floor(Math.random() * 3)]
				: srcIps[Math.floor(Math.random() * srcIps.length)],
		srcPort: 30000 + Math.floor(Math.random() * 35000),
		dst:
			i % 7 === 0
				? "203.0.113.42"
				: dstIps[Math.floor(Math.random() * dstIps.length)],
		dstPort: [443, 80, 53, 22, 853, 123, 25, 993, 21, 8080][
...
const generateDnsQueries = () => {
	...
	return Array.from({ length: 80 }, (_, i) => {
		...
		return {
			...
			latency:
				domain.status === "blocked" ? 0 : Math.floor(Math.random() * 50) + 5,
...
const dhcpLeases = [
	...
	{
		ip: "192.168.1.102",
		mac: "DC:A6:32:12:7B:4E",
		hostname: "pi-hole-backup",
		vendor: "Raspberry Pi",
		expires: "static",
		static: true,
		online: true,
	...
	{
		ip: "192.168.1.108",
		mac: "94:B8:6D:C2:48:9F",
		hostname: "iphone-lauren",
		vendor: "Apple Inc.",
		expires: "5h 47m",
		static: false,
		online: true,
	...
	{
		ip: "192.168.1.112",
		mac: "B4:2E:99:51:C8:3A",
		hostname: "selah-laptop",
		vendor: "HP Inc.",
		expires: "7h 02m",
		static: false,
		online: true,
...
const routers = [
	{
		...
		price: 149,
		image: "📦",
		specs: "Dual-core 1GHz, 512MB RAM, 4x GbE",
		plans: ["starter", "pro"],
	},
	{
		id: "br200",
	...
	{
		id: "br400",
		name: "NGFW.sh 400 Pro",
		price: 449,
		image: "📦",
		specs: "Quad-core 2GHz, 4GB RAM, 8x 2.5GbE, WiFi 6E, 10G SFP+",
		plans: ["business", "business_plus"],
	},
];
...
const Badge = ({
	...
	return (
		<span
			className={cn("px-2 py-0.5 text-xs font-mono rounded", colors[variant])}
		>
			{children}
		</span>
	);
...
const GaugeComponent = ({
	...
	const barColor =
		pct > 80 ? "bg-red-500" : pct > 60 ? "bg-amber-500" : colors[color];
	return (
...
const MiniChart = ({
	...
	const points = data
		.map(
			(d, i) =>
				`${(i / (data.length - 1)) * 100},${100 - ((d.value - min) / range) * 100}`,
		)
		.join(" ");
				...
				{data.map((row, i) => (
					...
					>
						{columns.map((c) => (
							<td
								key={c.key}
								className="px-3 py-2 font-mono text-zinc-300 whitespace-nowrap"
							>
...
const Button = ({
	...
	return (
		<button
			className={cn(
				"rounded font-medium transition-colors inline-flex items-center justify-center gap-1.5",
				variants[variant],
				sizes[size],
				className,
			)}
...
const Input = ({
	...
	<div className={className}>
		...
		<input
			className="w-full bg-zinc-800 border border-zinc-700 rounded px-3 py-1.5 text-sm text-zinc-200 font-mono placeholder:text-zinc-600 focus:outline-none focus:border-zinc-500"
			{...props}
		/>
	</div>
);
				...
				<Card title="Personal Information">
					<div className="space-y-4">
						...
						<p className="text-xs text-zinc-500">
							To update your profile, click the user icon above to open Clerk's
							profile manager.
						</p>
					</div>
				</Card>
							...
							<div className="flex items-center gap-3">
								<div className="w-8 h-8 bg-zinc-800 rounded flex items-center justify-center">
									<svg className="w-4 h-4" viewBox="0 0 24 24" fill="#4285F4">
										<path d="M12 0C5.37 0 0 5.37 0 12s5.37 12 12 12 12-5.37 12-12S18.63 0 12 0zm5.15 8.4h-4.3v1.6h4.2c-.2 2.2-2 3.8-4.2 3.8-2.4 0-4.3-1.9-4.3-4.3s1.9-4.3 4.3-4.3c1.1 0 2.1.4 2.9 1.1l1.2-1.2C15.7 4.1 14 3.4 12 3.4 8.3 3.4 5.3 6.4 5.3 10s3 6.6 6.7 6.6c3.9 0 6.5-2.7 6.5-6.5 0-.6 0-1-.1-1.4l-1.25-.3z" />
									</svg>
								</div>
								<div>
									<p className="text-sm text-zinc-200">Google</p>
									<p className="text-xs text-zinc-500">daniel@example.com</p>
								</div>
							</div>
							<Button size="sm" variant="ghost">
								Disconnect
					...
					<div className="space-y-4">
						...
						<div className="flex items-center justify-between">
							<span className="text-sm text-zinc-300">
								Threat Reports (Weekly)
							</span>
							<Toggle enabled={true} onChange={() => {}} />
						</div>
						<div className="flex items-center justify-between">
							<span className="text-sm text-zinc-300">Product Updates</span>
							<Toggle enabled={false} onChange={() => {}} />
						</div>
						<div className="flex items-center justify-between">
							<span className="text-sm text-zinc-300">Marketing Emails</span>
							<Toggle enabled={false} onChange={() => {}} />
						</div>
					</div>
				</Card>
	...
	return (
		<div className="space-y-8 max-w-6xl">
			...
			<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
				{billingPlans.map((plan) => (
					<div
						key={plan.id}
						className={cn(
							"bg-zinc-900 border rounded-xl p-5 relative transition-all",
							plan.popular
								? "border-emerald-500 ring-1 ring-emerald-500/20"
								: "border-zinc-800",
							plan.id === currentPlan && "ring-2 ring-emerald-500",
						)}
					>
						{plan.popular && (
						...
						<div className="mt-2 mb-4">
							<span className="text-3xl font-bold text-white">
								$
								{billingCycle === "annual"
									? Math.floor(plan.priceAnnual / 12 / 100)
									: Math.floor(plan.priceMonthly / 100)}
							</span>
							<span className="text-zinc-500">/mo</span>
							{billingCycle === "annual" && (
								<p className="text-xs text-zinc-500">
									billed annually (${Math.floor(plan.priceAnnual / 100)}/yr)
								</p>
							)}
						</div>
			...
			<Card
				...
				actions={
					<span className="text-xs text-zinc-500">
						One-time purchase with your plan
					</span>
				}
			>
					...
					<Table
						columns={[
							...
							{ key: "amount", label: "Amount" },
							{
								...
								label: "Status",
								render: (v) => (
									<Badge variant={v === "Paid" ? "success" : "warning"}>
										{v}
									</Badge>
								),
							},
			...
			<Card
				...
				className="bg-gradient-to-r from-zinc-900 to-zinc-900/50"
			>
...
const Dashboard = ({ onNavigate }: { onNavigate: (view: View) => void }) => {
	...
	const {
		...
		error: statusError,
	} = useDeviceStatus(firstDevice?.id ?? null);
	...
	const temp = hasLiveMetrics
		? (metrics!.temperature ?? systemStats.temp)
		: systemStats.temp;
	const load = hasLiveMetrics ? metrics!.load : systemStats.load;
	const connections = hasLiveMetrics
		? metrics!.connections
		: systemStats.connections;
	const uptime = hasLiveMetrics
				...
				<Card>
					<div className="flex flex-col items-center justify-center py-12 text-center">
						...
						<p className="text-sm text-zinc-500 mb-6 max-w-md">
							Register your first router to start monitoring and managing it
		...
		<div className="space-y-6">
			...
			{firstDevice && !isDeviceOnline && !statusLoading && !statusError && (
				<div className="bg-amber-900/20 border border-amber-800 rounded-lg px-4 py-3 flex items-center gap-2 text-sm text-amber-400">
					...
					Device &quot;{firstDevice.name}&quot; is offline. Showing last known
					metrics.
				</div>
			)}
				...
				<div className="flex items-center gap-2 text-xs text-zinc-500">
					<span
						className={cn(
							...
							isDeviceOnline ? "bg-emerald-500" : "bg-zinc-600",
						)}
					/>
					{isDeviceOnline
						? `Live from ${firstDevice.name}`
						: hasLiveMetrics
							? `Last known from ${firstDevice.name}`
							: `Mock data (loading ${firstDevice.name}...)`}
			...
			<div className="grid grid-cols-2 md:grid-cols-4 gap-4">
				...
				<Stat
					label="LAN Clients"
					value={systemStats.lanClients}
					sub="18 WiFi • 16 Wired"
			...
			<div className="grid grid-cols-1 lg:grid-cols-4 gap-4">
				<Card title="System Health" className="lg:col-span-1">
					<div className="space-y-4">
						...
						<div className="pt-2 border-t border-zinc-800">
							<div className="flex justify-between text-xs">
								<span className="text-zinc-500">Load Average</span>
				...
				<Card
					title="Bandwidth (24h)"
					actions={<span className="text-xs text-zinc-500">12.4 GB total</span>}
			...
			<div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
				<Card title="Active Connections (24h)" className="lg:col-span-1">
					...
					<div className="flex justify-between text-xs mt-2">
						<span className="text-zinc-500">Avg: 523</span>
						<span className="text-zinc-500">Peak: 1,247</span>
					</div>
				</Card>
				<Card title="DNS Queries (24h)" className="lg:col-span-1">
					<MiniChart data={dnsData} color="amber" height={80} />
					<div className="flex justify-between text-xs mt-2">
						<span className="text-zinc-500">Total: 48,293</span>
						<span className="text-zinc-500">Blocked: 18.2%</span>
					</div>
				</Card>
				<Card title="Threat Events (24h)" className="lg:col-span-1">
					<MiniChart data={threatData} color="red" height={80} />
					<div className="flex justify-between text-xs mt-2">
						<span className="text-zinc-500">Detected: 47</span>
						<span className="text-zinc-500">Blocked: 100%</span>
					</div>
				</Card>
			</div>
					...
					<Table
						columns={[
							{ key: "time", label: "Time" },
							{
								key: "type",
								label: "Type",
								render: (v) => <span className="text-red-400">{v}</span>,
							},
				...
				<Table
					columns={[
						...
						{ key: "tx", label: "TX Total", render: (v) => formatBytes(v) },
						{
							...
							label: "RX Rate",
							render: (v) => (
								<span className="text-emerald-400">{formatRate(v)}</span>
							),
						},
						{
							key: "txRate",
							label: "TX Rate",
							render: (v) => (
								<span className="text-blue-400">{formatRate(v)}</span>
							),
						},
					]}
					data={interfaces}
...
const TrafficPage = () => {
	...
	const filteredLogs = useMemo(() => {
		return trafficLogs.filter((log) => {
			if (filter.action !== "all" && log.action.toLowerCase() !== filter.action)
				return false;
			if (filter.proto !== "all" && log.proto.toLowerCase() !== filter.proto)
				return false;
			if (
				filter.search &&
				!`${log.src} ${log.dst} ${log.app}`
					.toLowerCase()
		...
		<div className="space-y-6">
			<div className="flex items-center justify-between">
				<div className="flex items-center gap-4">
					...
					<Select
						value={filter.proto}
						onChange={(e) =>
							setFilter((f) => ({ ...f, proto: e.target.value }))
						}
						options={[
				...
				<div className="flex gap-2">
					<Button
						size="sm"
						variant={liveMode ? "primary" : "default"}
			...
			<div className="grid grid-cols-2 md:grid-cols-4 gap-4">
				...
				<Stat
					label="Accepted"
					value={formatNumber(
						filteredLogs.filter((l) => l.action === "ACCEPT").length,
					)}
				...
				<Stat
					...
					value={formatNumber(filteredLogs.filter((l) => l.threat).length)}
			...
			<Card>
						...
						<thead>
							<tr className="border-b border-zinc-800 text-left">
								{[
									...
									<th key={h} className="px-2 py-2 text-zinc-500 font-medium">
										{h}
									</th>
								))}
							</tr>
							...
							{filteredLogs.slice(0, 50).map((log, i) => (
								<tr key={i} className="hover:bg-zinc-800/30 cursor-pointer">
									...
									<td className="px-2 py-1.5">
										<Badge>{log.proto}</Badge>
									</td>
									<td className="px-2 py-1.5 text-zinc-500">{log.flags}</td>
									<td className="px-2 py-1.5 text-zinc-400">
										{formatNumber(log.bytes)}
									</td>
									<td className="px-2 py-1.5 text-zinc-500">{log.packets}</td>
									<td className="px-2 py-1.5 text-blue-400">{log.app}</td>
									<td className="px-2 py-1.5">
										<Badge variant="info">{log.geo}</Badge>
									</td>
									<td className="px-2 py-1.5">
										{log.threat && <Badge variant="danger">{log.threat}</Badge>}
									</td>
									<td className="px-2 py-1.5">
										<Badge
											variant={log.action === "ACCEPT" ? "success" : "danger"}
										>
											{log.action}
										</Badge>
									</td>
				...
				<div className="flex items-center justify-between pt-4 border-t border-zinc-800 mt-4">
					<span className="text-xs text-zinc-500">
						Showing {Math.min(50, filteredLogs.length)} of {filteredLogs.length}{" "}
						entries
					</span>
					<div className="flex gap-2">
...
const DNSFilterPage = () => {
	...
	const filteredQueries = useMemo(() => {
		return dnsQueries.filter((q) => {
			...
			if (
				filter.search &&
				!q.query.toLowerCase().includes(filter.search.toLowerCase())
			)
				return false;
			return true;
		});
		...
		<div className="space-y-6">
			<div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
				<Stat
					...
					value={formatNumber(stats.total * 150)}
					icon={<Database className="w-4 h-4" />}
				/>
				<Stat
					label="Blocked"
					value={formatNumber(stats.blocked * 150)}
					sub={`${((stats.blocked / stats.total) * 100).toFixed(1)}%`}
					icon={<ShieldOff className="w-4 h-4" />}
				/>
				<Stat
					label="Cached"
					value={formatNumber(stats.cached * 150)}
					sub={`${((stats.cached / stats.total) * 100).toFixed(1)}%`}
						...
						columns={[
							{
								key: "enabled",
								label: "",
								render: (v) => <Toggle enabled={v} onChange={() => {}} />,
							},
							{ key: "name", label: "Name" },
							{
								key: "entries",
							...
							{ key: "updated", label: "Updated" },
							{
								...
								label: "Blocked Today",
								render: (v) => (
									<span className="text-red-400">{formatNumber(v)}</span>
								),
							},
						]}
						data={[
				...
				<Table
					columns={[
						...
						},
						{
							...
							label: "Status",
							render: (v) => (
								<Badge variant={v === "blocked" ? "danger" : "success"}>
									{v}
								</Badge>
							),
						},
				...
				<Card title="Connection Status" className="lg:col-span-1">
					<div className="space-y-4">
						<div className="flex items-center gap-3">
							...
							<span className="text-lg text-zinc-200">Connected</span>
						</div>
						<div className="space-y-2 text-sm">
							<div className="flex justify-between">
								<span className="text-zinc-500">IP Address</span>
								<span className="font-mono text-zinc-200">203.0.113.42</span>
							</div>
							<div className="flex justify-between">
								<span className="text-zinc-500">Gateway</span>
								<span className="font-mono text-zinc-200">203.0.113.1</span>
							</div>
							<div className="flex justify-between">
								<span className="text-zinc-500">DNS Servers</span>
							...
							<div className="flex justify-between">
								...
								<span className="font-mono text-zinc-200">255.255.255.0</span>
							</div>
							<div className="flex justify-between">
							...
							<div className="flex justify-between">
								...
								<span className="font-mono text-zinc-200">14d 7h 23m</span>
							</div>
						</div>
				...
				<div className="space-y-4">
					<div className="flex items-center gap-4">
						<Toggle
							enabled={false}
							onChange={() => {}}
							label="Enable Dual-WAN"
						/>
						<Select
							options={[
								{ value: "failover", label: "Failover Mode" },
								{ value: "loadbalance", label: "Load Balance" },
							]}
							className="w-48"
						/>
					</div>
					<Table
						columns={[
							{ key: "interface", label: "Interface" },
							{
								...
								label: "Status",
								render: (v) => (
									<Badge variant={v === "Primary" ? "success" : "info"}>
										{v}
									</Badge>
								),
							},
	...
	<div className="space-y-6">
					...
					<div className="space-y-3">
						...
						<div className="space-y-1 text-xs">
							<div className="flex justify-between">
								...
								<span className="text-zinc-300">{net.band}</span>
							</div>
							<div className="flex justify-between">
								<span className="text-zinc-500">Channel</span>
								<span className="text-zinc-300">
									{net.channel} ({net.width})
								</span>
							</div>
							<div className="flex justify-between">
								<span className="text-zinc-500">Security</span>
								<Badge variant="info">{net.security}</Badge>
							</div>
							<div className="flex justify-between">
								<span className="text-zinc-500">Clients</span>
								<span className="text-zinc-300">{net.clients}</span>
							</div>
							{net.vlan && (
								<div className="flex justify-between">
									<span className="text-zinc-500">VLAN</span>
									<span className="text-zinc-300">{net.vlan}</span>
								</div>
							)}
						</div>
		...
		<Card title="Radio Settings">
			...
			<div className="flex justify-end pt-4 mt-4 border-t border-zinc-800">
				<Button variant="primary">Apply Radio Settings</Button>
			</div>
		</Card>
	</div>
);
			...
			<Table
				columns={[
					...
					{ key: "vendor", label: "Vendor" },
					{
						...
						label: "Expires",
						render: (v, r) =>
							r.static ? <Badge variant="info">Static</Badge> : v,
					},
					{
						key: "online",
						label: "Status",
					...
					},
					{
						...
						render: () => (
							<div className="flex gap-1">
								<Button size="sm" variant="ghost" title="Make Static">
									<Lock className="w-3 h-3" />
								</Button>
								<Button size="sm" variant="ghost" title="Wake">
									<Power className="w-3 h-3" />
								</Button>
							</div>
						),
			...
			<div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
				<div className="space-y-3">
					...
					<div className="flex justify-between text-sm">
						...
						<span className="font-mono text-zinc-200">
							2024-12-15 14:23:47 UTC
						</span>
					</div>
					<div className="flex justify-between text-sm">
						<span className="text-zinc-500">Kernel</span>
						<span className="font-mono text-zinc-200">6.1.72-edge</span>
					</div>
					<div className="flex justify-between text-sm">
						<span className="text-zinc-500">Architecture</span>
						<span className="font-mono text-zinc-200">
							aarch64 (ARM Cortex-A53)
						</span>
					</div>
					<div className="flex justify-between text-sm">
						<span className="text-zinc-500">Boot Slot</span>
						<Badge variant="success">A (Active)</Badge>
					</div>
				</div>
				<div className="space-y-3">
			...
			<Table
				columns={[
					...
					},
					{
						...
						render: (_, r) =>
							r.status !== "Active" && (
								<Button size="sm">Boot This Slot</Button>
							),
					},
				]}
				data={[
		...
		<Card title="Manual Upload">
			<div className="border-2 border-dashed border-zinc-700 rounded-lg p-8 text-center">
				...
				<p className="text-sm text-zinc-400 mb-2">
					Drag and drop firmware image or click to browse
				</p>
				<p className="text-xs text-zinc-600">
					Supported formats: .bin, .img, .tar.gz (GPG signed)
				</p>
	...
	<div className="space-y-6">
			...
			<Card title="Export Configuration">
				<div className="space-y-4">
					<p className="text-sm text-zinc-400">
						Download a complete backup of your router configuration.
					</p>
					<div className="space-y-2">
						<Toggle
							enabled={true}
							onChange={() => {}}
							label="Include WiFi passwords"
						/>
						<Toggle
							enabled={true}
							onChange={() => {}}
							label="Include VPN keys"
						/>
						<Toggle
							enabled={false}
							onChange={() => {}}
					...
					<Input
						...
						type="password"
						placeholder="Enter password if encrypting"
						disabled
					/>
			...
			<Card title="Restore Configuration">
				<div className="space-y-4">
					<div className="border-2 border-dashed border-zinc-700 rounded-lg p-6 text-center">
						...
						<p className="text-sm text-zinc-400">
							Drop config file or click to browse
						</p>
					</div>
					<Input
						label="Decryption Password"
						type="password"
						placeholder="If backup is encrypted"
					/>
		...
		<Card title="Saved Backups">
			<Table
				...
				data={[
					{
						name: "config-2024-12-20.json",
						date: "2024-12-20 14:30",
						size: "24 KB",
						encrypted: false,
					},
					{
						name: "config-2024-12-15.json.enc",
						date: "2024-12-15 09:15",
						size: "28 KB",
						encrypted: true,
					},
					{
						name: "config-2024-12-01.json",
...
const HardwarePage = () => (
	<div className="space-y-6">
		<div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
			...
			<Stat
				label="Load Average"
				value={systemStats.load[0].toFixed(2)}
				sub={systemStats.load.join(" / ")}
			/>
		</div>
			...
			<Table
				columns={[
					...
					{ key: "duplex", label: "Duplex" },
					{
						...
						label: "Link",
						render: (v) => (
							<Badge variant={v === "up" ? "success" : "default"}>{v}</Badge>
						),
					},
				]}
				data={[
				...
				<div className="space-y-3">
					{["Power", "WAN", "LAN", "WiFi 5G", "WiFi 2G"].map((led) => (
						<div key={led} className="flex items-center justify-between">
							<span className="text-sm text-zinc-300">{led}</span>
							<Toggle enabled={true} onChange={() => {}} />
						</div>
					))}
				</div>
	...
	<div className="space-y-6">
		<Card title="Grafana Configuration">
			<div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
				<Input
					label="Grafana URL"
					defaultValue="https://grafana.local:3000"
					className="lg:col-span-2"
				/>
				<Input
					label="API Token"
	...
	<div className="space-y-6">
		<Card title="Loki Configuration">
			<div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
				<Input
					label="Loki URL"
					defaultValue="https://loki.local:3100"
					className="lg:col-span-2"
				/>
				<Input
					label="Auth Header"
			...
			<div className="space-y-4">
				...
				<div className="bg-zinc-950 rounded border border-zinc-800 p-4 font-mono text-xs h-96 overflow-auto">
					<div className="space-y-1 text-zinc-400">
						<div>
							<span className="text-zinc-600">2024-12-25T14:23:47.293Z</span>{" "}
							<span className="text-emerald-500">INFO</span> [firewall] ACCEPT
							src=192.168.1.105 dst=142.250.185.78:443 proto=TCP
						</div>
						<div>
							<span className="text-zinc-600">2024-12-25T14:23:46.982Z</span>{" "}
							<span className="text-red-500">WARN</span> [firewall] DROP
							src=203.0.113.89 dst=203.0.113.42:22 proto=TCP flags=SYN
						</div>
						<div>
							<span className="text-zinc-600">2024-12-25T14:23:46.847Z</span>{" "}
							<span className="text-emerald-500">INFO</span> [dns] RESOLVE
							client=192.168.1.108 query=github.com answer=140.82.112.3
							latency=8ms
						</div>
						<div>
							<span className="text-zinc-600">2024-12-25T14:23:46.729Z</span>{" "}
							<span className="text-amber-500">BLOCK</span> [dns] BLOCKED
							client=192.168.1.112 query=ads.doubleclick.net reason=blocklist
						</div>
						<div>
							<span className="text-zinc-600">2024-12-25T14:23:46.612Z</span>{" "}
							<span className="text-red-500">WARN</span> [firewall] DROP
							src=45.33.32.156 dst=203.0.113.42:23 proto=TCP
							reason=telnet-blocked
						</div>
						<div>
							<span className="text-zinc-600">2024-12-25T14:23:46.501Z</span>{" "}
							<span className="text-emerald-500">INFO</span> [dhcp] LEASE
							ip=192.168.1.142 mac=66:77:88:99:AA:BB hostname=ring-doorbell
						</div>
						<div>
							<span className="text-zinc-600">2024-12-25T14:23:46.388Z</span>{" "}
							<span className="text-emerald-500">INFO</span> [firewall] ACCEPT
							src=192.168.1.125 dst=104.16.132.229:443 proto=TCP
						</div>
						<div>
							<span className="text-zinc-600">2024-12-25T14:23:46.274Z</span>{" "}
							<span className="text-blue-500">DEBUG</span> [system] CPU=23%
							MEM=41% TEMP=52C LOAD=0.42
						</div>
					</div>
				</div>
			</div>
...
const FirewallPage = () => {
	...
	const filteredRules = useMemo(() => {
		return firewallRules.filter((rule) => {
			if (
				filter.zone !== "all" &&
				!rule.zone.toLowerCase().includes(filter.zone.toLowerCase())
			)
				return false;
			if (
				filter.search &&
				...
				<Card>
					...
					<div className="bg-red-900/20 border border-red-800 rounded-lg px-4 py-3 mb-6 flex items-start gap-2">
						...
						<p className="text-sm text-red-400">
							This is the only time the API key will be displayed. Copy and
							store it securely.
						</p>
					</div>
					...
					<div className="space-y-4">
						<div>
							...
							<div className="flex items-center gap-2">
								<code className="flex-1 bg-zinc-800 border border-zinc-700 rounded px-3 py-2 text-sm font-mono text-zinc-200 break-all">
								...
								<Button
									...
									variant="ghost"
									onClick={() =>
										copyToClipboard(registrationResult.device_id, "device_id")
									}
								>
						...
						<div>
							...
							<div className="flex items-center gap-2">
								<code className="flex-1 bg-zinc-800 border border-zinc-700 rounded px-3 py-2 text-sm font-mono text-emerald-400 break-all">
								...
								<Button
									...
									variant="ghost"
									onClick={() =>
										copyToClipboard(registrationResult.api_key, "api_key")
									}
								>
									{copiedField === "api_key" ? (
										<Check className="w-3 h-3 text-emerald-400" />
						...
						<div>
							...
							<div className="flex items-center gap-2">
								<code className="flex-1 bg-zinc-800 border border-zinc-700 rounded px-3 py-2 text-sm font-mono text-zinc-200 break-all">
									{registrationResult.websocket_url}
								</code>
								<Button
									size="sm"
									variant="ghost"
									onClick={() =>
										copyToClipboard(registrationResult.websocket_url, "ws_url")
									}
								>
									{copiedField === "ws_url" ? (
										<Check className="w-3 h-3 text-emerald-400" />
					...
					<div className="mt-6 pt-6 border-t border-zinc-800">
						...
						<p className="text-xs text-zinc-500 mb-3">
							Add this to your router agent's{" "}
							<code className="text-zinc-400">config.toml</code>:
						</p>
					...
					<div className="space-y-4">
						...
						<div className="flex justify-end gap-2">
							...
							<Button
								...
								disabled={registering || !formName.trim()}
							>
								{registering ? (
									<>
										<Spinner /> Registering...
						...
						>
							...
							<div className="space-y-1.5 text-xs mb-4">
								...
								<div className="flex justify-between">
									...
									<span className="font-mono text-zinc-400 truncate ml-4 max-w-[140px]">
										{device.id}
									</span>
								</div>
								<div className="flex justify-between">
...
export default function App() {
	...
	const currentLabel =
		navGroups.flatMap((g) => g.items).find((i) => i.id === view)?.label ||
		"Dashboard";
					...
					>
							...
							{navGroups.map((group) => (
								<div key={group.label}>
									<button
										...
										className="w-full flex items-center justify-between px-4 py-2 text-xs font-medium text-zinc-500 uppercase tracking-wider hover:text-zinc-300"
									>
										{group.label}
						...
						<div className="p-3 border-t border-zinc-800">
							...
							<button
								...
								className="w-full flex items-center gap-2 px-2 py-1.5 text-xs text-zinc-400 hover:text-zinc-200 hover:bg-zinc-800 rounded transition-colors"
							>
