...
test.describe('Device Deletion', () => {
  ...
  test('should show confirmation dialog when clicking delete button', async ({ page }) => {
    ...
    // Click delete button
    await page.click('[data-device-id="device-123"] button[aria-label="Delete device"]');
  ...
  test('should show error message on deletion failure', async ({ page }) => {
    await page.route('**/fleet/devices/device-123', async (route) => {
      if (route.request().method() === 'DELETE') {
        await route.fulfill({
          status: 403,
          contentType: 'application/json',
          body: JSON.stringify({ error: 'Cannot delete device owned by another user' }),
        });
      }
    });
    ...
    await expect(page.locator('text=Cannot delete device owned by another user')).toBeVisible();
