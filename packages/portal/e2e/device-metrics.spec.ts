...
test.describe("Device Metrics", () => {
	...
	test("should display device metrics when device is online", async ({
		page,
	}) => {
	...
	test("should stop polling when switching to different device", async ({
		page,
	}) => {
		const mockDevice2 = {
			...mockDevice,
	...
	test("should show error message on metrics fetch failure", async ({
		page,
	}) => {
		await page.route("**/fleet/devices/device-123/status", async (route) => {
			await route.fulfill({
	...
	test("should recover from temporary errors and continue polling", async ({
		page,
	}) => {
		let requestCount = 0;
