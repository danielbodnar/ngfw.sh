...
test.describe("Device List", () => {
	...
	test("should show empty state when no devices", async ({ page }) => {
		...
		await expect(
			page.locator('button:has-text("Register Device")'),
		).toBeVisible();
	});

	test("should display device list with correct information", async ({
		page,
	}) => {
		const mockDevices = [
			{
		...
		await expect(
			page.locator('[data-device-id="device-1"] [data-status="online"]'),
		).toBeVisible();
		await expect(
			page.locator('[data-device-id="device-2"] [data-status="provisioning"]'),
		).toBeVisible();
		await expect(
			page.locator('[data-device-id="device-3"] [data-status="offline"]'),
		).toBeVisible();
	});
	...
	test("should refresh device list when clicking refresh button", async ({
		page,
	}) => {
		let requestCount = 0;
