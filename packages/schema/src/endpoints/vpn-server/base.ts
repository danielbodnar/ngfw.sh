...
export const vpnServerConfig = z.object({
	...
	listen_port: z
		...
		.max(65535)
		.default(51820)
		.describe("UDP port for WireGuard"),
	network: z
		.string()
		.describe("VPN network in CIDR notation (e.g., 10.0.0.0/24)"),
	public_key: z.string().describe("Server public key (base64)"),
	private_key: z
		.string()
		.describe("Server private key (base64) - never returned in GET"),
	...
	mtu: z
		...
		.default(1420)
		.optional()
		.describe("MTU for WireGuard interface"),
	persistent_keepalive: z
		...
		.default(25)
		.optional()
		.describe("Persistent keepalive interval in seconds"),
});

/**
 * Public WireGuard server configuration (without private_key)
 */
export const vpnServerConfigPublic = vpnServerConfig.omit({
	private_key: true,
});
...
export const vpnServerPeer = z.object({
	...
	name: z
		...
		.min(1)
		.max(64)
		.describe("Peer name (e.g., 'Phone', 'Laptop')"),
	public_key: z.string().describe("Peer public key (base64)"),
	preshared_key: z
		.string()
		.optional()
		.describe("Optional preshared key for additional security (base64)"),
	allowed_ips: z
		.array(z.string())
		.describe("IP addresses allowed for this peer (e.g., ['10.0.0.2/32'])"),
	endpoint: z
		.string()
		.optional()
		.describe(
			"Peer endpoint (IP:port) - usually not set for road warrior configs",
		),
	persistent_keepalive: z
		.number()
		.int()
	...
	last_handshake: z
		...
		.nullable()
		.optional()
		.describe("Last successful handshake timestamp"),
