...
export interface ApiClient {
	...
	updateVPNServerConfig(
		deviceId: string,
		config: Partial<VPNServerConfig>,
	...
	createVPNClientProfile(
		profile: Partial<VPNClientProfile>,
	...
	return {
		...
		listRoutes(deviceId) {
			return request<Route[]>(
				`/routing/routes?device_id=${encodeURIComponent(deviceId)}`,
			);
		},
		...
		listNATRules(deviceId) {
			return request<NATRule[]>(
				`/nat/rules?device_id=${encodeURIComponent(deviceId)}`,
			);
		},
		...
		getIPSConfig(deviceId) {
			return request<IPSConfig>(
				`/ips/config?device_id=${encodeURIComponent(deviceId)}`,
			);
		},
		...
		listIPSRules(deviceId) {
			return request<IPSRule[]>(
				`/ips/rules?device_id=${encodeURIComponent(deviceId)}`,
			);
		},

		listIPSAlerts(deviceId) {
			return request<IPSAlert[]>(
				`/ips/alerts?device_id=${encodeURIComponent(deviceId)}`,
			);
		},

		// VPN Server
		getVPNServerConfig(deviceId) {
			return request<VPNServerConfig>(
				`/vpn/server/config?device_id=${encodeURIComponent(deviceId)}`,
			);
		},
		...
		listVPNServerPeers(deviceId) {
			return request<VPNServerPeer[]>(
				`/vpn/server/peers?device_id=${encodeURIComponent(deviceId)}`,
			);
		},
		...
		listVPNClientProfiles(deviceId) {
			return request<VPNClientProfile[]>(
				`/vpn/client/profiles?device_id=${encodeURIComponent(deviceId)}`,
			);
		},
		...
		updateVPNClientProfile(profileId, profile) {
			return request<VPNClientProfile>(
				`/vpn/client/profiles/${encodeURIComponent(profileId)}`,
				{
					method: "PUT",
					body: JSON.stringify(profile),
				},
		...
		getVPNClientStatus(profileId) {
			return request<VPNClientStatus>(
				`/vpn/client/profiles/${encodeURIComponent(profileId)}/status`,
			);
		},

		connectVPNClient(profileId) {
			return request<void>(
				`/vpn/client/profiles/${encodeURIComponent(profileId)}/connect`,
				{
					method: "POST",
				},
			);
		...
		disconnectVPNClient(profileId) {
			return request<void>(
				`/vpn/client/profiles/${encodeURIComponent(profileId)}/disconnect`,
				{
					method: "POST",
				},
			);
		...
		listQoSRules(deviceId) {
			return request<QoSRule[]>(
				`/qos/rules?device_id=${encodeURIComponent(deviceId)}`,
			);
		},
		...
		listDDNSConfigs(deviceId) {
			return request<DDNSConfig[]>(
				`/ddns/configs?device_id=${encodeURIComponent(deviceId)}`,
			);
		},
		...
		updateDDNSConfig(configId, data) {
			return request<DDNSConfig>(
				`/ddns/configs/${encodeURIComponent(configId)}`,
				{
					method: "PUT",
					body: JSON.stringify(data),
				},
		...
		forceDDNSUpdate(configId) {
			return request<void>(
				`/ddns/configs/${encodeURIComponent(configId)}/update`,
				{
					method: "POST",
				},
			);
		...
		queryLogs(query) {
			...
			if (query.start_time)
				params.set("start_time", query.start_time.toString());
