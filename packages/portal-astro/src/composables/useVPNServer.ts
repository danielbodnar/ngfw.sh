...
import type { VPNServerConfig, VPNServerPeer } from '../lib/api/types';
...
export function useVPNServer(
  ...
  async function refetch(): Promise<void> {
    ...
    try {
      ...
      error.value = err instanceof Error ? err.message : 'Failed to fetch VPN server data';
    } finally {
      loading.value = false;
    }
  }

  async function updateConfig(updates: Partial<VPNServerConfig>): Promise<VPNServerConfig> {
    if (!deviceId.value) {
    ...
    try {
      ...
      error.value = err instanceof Error ? err.message : 'Failed to update VPN server config';
      throw err;
    }
  }

  async function createPeer(peer: Partial<VPNServerPeer>): Promise<VPNServerPeer> {
    if (!deviceId.value) {
      throw new Error('No device ID provided');
    }
    ...
    try {
      ...
      error.value = err instanceof Error ? err.message : 'Failed to create peer';
      throw err;
    }
  }
  ...
  async function deletePeer(peerId: string): Promise<void> {
    ...
    try {
      ...
      error.value = err instanceof Error ? err.message : 'Failed to delete peer';
      throw err;
    }
  }
