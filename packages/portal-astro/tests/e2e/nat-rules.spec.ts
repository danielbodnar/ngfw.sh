...
test.describe('NAT Rules Management', () => {
  ...
  test('creates new NAT rule', async ({ page }) => {
    ...
    await page.fill('textarea[name="description"]', 'Email server port forward');
  ...
  test('edits existing NAT rule', async ({ page }) => {
    ...
    await expect(page.locator('input[name="name"]')).toHaveValue('SSH Port Forward');
  ...
  test('refreshes rules list', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="spinner"]')).toBeVisible({ timeout: 1000 });
