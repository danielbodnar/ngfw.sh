...
test.describe("Onboarding Flow", () => {
	test("completes full onboarding journey", async ({ page }) => {
		...
		await expect(
			page.locator("text=/Select.*Router|Choose.*Device/i"),
		).toBeVisible();
		...
		await expect(
			page.locator("text=/Register.*Device|Device.*Setup/i"),
		).toBeVisible();
		...
		await expect(
			page.locator("text=/Initial.*Configuration|Basic.*Setup/i"),
		).toBeVisible();
		...
		await expect(
			page.locator("text=/Setup.*Complete|All.*Set/i"),
		).toBeVisible();
	...
	test("shows validation errors in device registration", async ({
		page,
		context,
	}) => {
	...
	test("shows helpful tooltips and documentation", async ({
		page,
		context,
	}) => {
		await context.addCookies([
		...
		if (await helpButton.isVisible()) {
			...
			await expect(
				page.locator('[role="tooltip"], [role="dialog"]'),
			).toBeVisible();
		}
	});

	test("navigates backward through onboarding steps", async ({
		page,
		context,
	}) => {
		await context.addCookies([
	...
	test("displays router models with images and specs", async ({
		page,
		context,
	}) => {
		await context.addCookies([
		...
		await expect(page.locator('[data-testid^="router-"]')).toHaveCount(3, {
			timeout: 5000,
		});
