...
function log(msg_type: string, device_id: string | null, summary: string): void {
...
const server = Bun.serve<ConnectionState>({
  ...
  websocket: {
    open(ws) {
      ...
      log("CONNECT", device_id, `WebSocket opened (owner_id=${ws.data.owner_id})`);
...
function handleAuth(ws: { data: ConnectionState; send: (msg: string) => void }, msg: RpcMessage): void {
  const payload = msg.payload as { device_id?: string; api_key?: string; firmware_version?: string };
  const { device_id, api_key, firmware_version } = payload;

  log("AUTH", ws.data.device_id, `device_id=${device_id} firmware=${firmware_version}`);
...
function handleStatus(ws: { data: ConnectionState; send: (msg: string) => void }, msg: RpcMessage): void {
  log("STATUS", ws.data.device_id, `cpu=${msg.payload.cpu} mem=${msg.payload.memory} uptime=${msg.payload.uptime}`);
  ws.data.last_status = msg.payload;

  ws.send(response("STATUS_OK", {}, msg.id));
}

function handleMetrics(ws: { data: ConnectionState; send: (msg: string) => void }, msg: RpcMessage): void {
  log("METRICS", ws.data.device_id, `cpu=${msg.payload.cpu} mem=${msg.payload.memory} ts=${msg.payload.timestamp}`);
  ws.data.last_metrics = msg.payload;
}

function handlePing(ws: { data: ConnectionState; send: (msg: string) => void }, msg: RpcMessage): void {
...
function handleLog(ws: { data: ConnectionState }, msg: RpcMessage): void {
  const payload = msg.payload as { level?: string; component?: string; message?: string };
  log("LOG", ws.data.device_id, `[${payload.level}] ${payload.component}: ${payload.message}`);
}

function handleAlert(ws: { data: ConnectionState }, msg: RpcMessage): void {
  const payload = msg.payload as { severity?: string; alert_type?: string; description?: string };
  log("ALERT", ws.data.device_id, `[${payload.severity}] ${payload.alert_type}: ${payload.description}`);
}

console.log(`NGFW Mock API server listening on http://localhost:${PORT}`);
console.log(`  WebSocket: ws://localhost:${PORT}/agent/ws?device_id=...&owner_id=...`);
