...
export class MockApiServer {
	...
	private messageHandlers: Map<string, (msg: ProtocolMessage) => void> =
		new Map();
	...
	async start(): Promise<void> {
		this.server = Bun.serve({
			...
			fetch: async (req, server) => {
				...
				if (url.pathname === "/agent/ws") {
					...
					if (!deviceId || !ownerId) {
						return new Response("Missing device_id or owner_id", {
							status: 400,
						});
					}
			...
			websocket: {
				open: (ws) => {
					...
					if (this.config.verbose) {
						console.log(
							`[WS] Connection opened: ${connectionId} (device: ${ws.data.deviceId})`,
						);
					}
				},
		...
		if (this.config.verbose) {
			console.log(
				`Mock API server listening on http://${this.config.host}:${this.config.port}`,
			);
		}
	}
	...
	private handleWebSocketMessage(ws: any, message: string | Buffer): void {
		try {
			...
			if (!connectionEntry) {
				console.error(
					"[WS] Connection not found for device:",
					ws.data.deviceId,
				);
				return;
			}
			...
			if (this.config.verbose) {
				console.log(
					`[WS] ${timestamp.toISOString()} Received: ${msg.type}`,
					msg.payload,
				);
			}
	...
	private handleAuth(
		ws: any,
		msg: ProtocolMessage<AuthPayload>,
