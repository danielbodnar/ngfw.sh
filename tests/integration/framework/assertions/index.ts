 ...
 */
class Assertion {
  protected createResult(passed: boolean, message: string, actual?: unknown, expected?: unknown): AssertionResult {
...
export class WebSocketAssertions extends Assertion {
   ...
   */
  assertMessageType(message: MockMessage | ProtocolMessage, expectedType: string): AssertionResult {
    if (message.type !== expectedType) {
      this.fail(`Expected message type ${expectedType}, got ${message.type}`, message.type, expectedType);
    }
   ...
   */
  assertMessagePayload(message: MockMessage | ProtocolMessage, expectedFields: Record<string, unknown>): AssertionResult {
    const payload = message.payload as Record<string, unknown>;

    for (const [key, expectedValue] of Object.entries(expectedFields)) {
      if (!(key in payload)) {
        this.fail(`Expected payload to have field ${key}`, payload, expectedFields);
      }

      if (expectedValue !== undefined && payload[key] !== expectedValue) {
        this.fail(`Expected payload.${key} to be ${expectedValue}, got ${payload[key]}`, payload[key], expectedValue);
      }
    }
  ...
  assertAuthFailure(message: MockMessage | ProtocolMessage): AssertionResult {
    ...
    if (payload.success !== false) {
      this.fail('Expected auth failure, but payload.success is not false', payload);
    }
   ...
   */
  assertMessageOrder(messages: MockMessage[], expectedTypes: string[]): AssertionResult {
...
export class StateAssertions extends Assertion {
   ...
   */
  assertDeviceStatus(device: DeviceFixture, expectedStatus: DeviceFixture['status']): AssertionResult {
    if (device.status !== expectedStatus) {
      this.fail(`Expected device status ${expectedStatus}, got ${device.status}`, device.status, expectedStatus);
    }
   ...
   */
  assertDeviceRecentlySeen(device: DeviceFixture, maxAge: number = 60000): AssertionResult {
    if (!device.last_seen) {
      this.fail('Device has never been seen', device.last_seen, 'recent timestamp');
    }
  ...
  assertMetricsInRange(
    ...
    if (ranges.cpu) {
      ...
      if (metrics.cpu < min || metrics.cpu > max) {
        this.fail(`CPU usage ${metrics.cpu}% is outside range [${min}, ${max}]`, metrics.cpu, ranges.cpu);
      }
    }
    ...
    if (ranges.memory) {
      ...
      if (metrics.memory < min || metrics.memory > max) {
        this.fail(`Memory usage ${metrics.memory}% is outside range [${min}, ${max}]`, metrics.memory, ranges.memory);
      }
    }
...
export class StorageAssertions extends Assertion {
   ...
   */
  assertKeyNotExists(storage: Map<string, unknown>, key: string): AssertionResult {
    if (storage.has(key)) {
      this.fail(`Expected key ${key} to not exist in storage`, storage.get(key), undefined);
    }
   ...
   */
  assertStorageValue(storage: Map<string, unknown>, key: string, expectedValue: unknown): AssertionResult {
    this.assertKeyExists(storage, key);

    const actualValue = storage.get(key);
    if (JSON.stringify(actualValue) !== JSON.stringify(expectedValue)) {
      this.fail(`Expected storage[${key}] to be ${JSON.stringify(expectedValue)}`, actualValue, expectedValue);
    }

    return this.createResult(true, `Storage contains expected value for ${key}`);
  }
  ...
  assertStorageEmpty(storage: Map<string, unknown>): AssertionResult {
    if (storage.size > 0) {
      this.fail(`Expected storage to be empty, but it has ${storage.size} keys`, storage.size, 0);
    }
   ...
   */
  assertStorageSize(storage: Map<string, unknown>, expectedSize: number): AssertionResult {
    if (storage.size !== expectedSize) {
      this.fail(`Expected storage size to be ${expectedSize}, got ${storage.size}`, storage.size, expectedSize);
    }
...
export class TimingAssertions extends Assertion {
   ...
   */
  async assertCompletesWithin<T>(operation: () => Promise<T>, maxDuration: number): Promise<T> {
    ...
    if (duration > maxDuration) {
      this.fail(`Operation took ${duration}ms (max ${maxDuration}ms)`, duration, maxDuration);
    }
   ...
   */
  async assertTakesAtLeast<T>(operation: () => Promise<T>, minDuration: number): Promise<T> {
    const startTime = Date.now();
    ...
    if (duration < minDuration) {
      this.fail(`Operation took ${duration}ms (min ${minDuration}ms)`, duration, minDuration);
    }
   ...
   */
  assertIntervals(timestamps: number[], expectedInterval: number, tolerance: number = 100): AssertionResult {
    if (timestamps.length < 2) {
      this.fail('Need at least 2 timestamps to check intervals', timestamps.length, 2);
    }
    ...
    return this.createResult(true, `All intervals are within ${tolerance}ms of ${expectedInterval}ms`);
  }
}
