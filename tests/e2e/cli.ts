...
import { testSuites, getSmokeSuites, getPrerequisiteSuites, getPerformanceSuites } from "./suites";
...
function listSuites(): void {
  ...
  for (const [category, suites] of Array.from(grouped.entries()).sort()) {
    ...
    for (const suite of suites) {
      const deps = suite.dependencies.length > 0 ? ` (deps: ${suite.dependencies.length})` : "";
      const parallel = suite.parallel ? " [parallel]" : " [sequential]";
      console.log(`  ${suite.id.padEnd(40)} ${suite.environment.padEnd(8)} ${suite.timeout / 1000}s${deps}${parallel}`);
