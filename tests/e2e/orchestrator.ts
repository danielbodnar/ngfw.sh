...
export class E2EOrchestrator {
  ...
  async run(): Promise<TestResult[]> {
    ...
    this.log(`Running ${filtered.length} of ${this.suites.length} registered suites`);
    ...
    return this.suites.filter((suite) => {
      ...
      if (this.context.config.environments.length > 0) {
        const envMatch = this.context.config.environments.some(
          (env) => env === "both" || env === suite.environment || suite.environment === "both"
        );
      ...
      if (this.context.config.tags.length > 0) {
        const hasTag = this.context.config.tags.some((tag) => suite.tags.includes(tag));
        if (!hasTag) return false;
      }

      // Filter by excluded tags
      if (this.context.config.excludeTags.length > 0) {
        const hasExcludedTag = this.context.config.excludeTags.some((tag) => suite.tags.includes(tag));
  ...
  private buildExecutionPlan(suites: TestSuite[]): TestSuite[][] {
    ...
    while (remaining.size > 0 && safety < maxIterations) {
      ...
      for (const [id, suite] of remaining) {
        ...
        const depsResolved = suite.dependencies.every((dep) => resolved.has(dep));
    ...
    while (attempts < maxAttempts) {
      ...
      try {
        ...
        if (attempts < maxAttempts) {
          this.log(`✗ Failed: ${suite.name} (attempt ${attempts}/${maxAttempts}), retrying...`);
  ...
  private async cleanupTestInfrastructure(): Promise<void> {
    ...
    try {
      execSync(`docker compose -f ${join(PROJECT_ROOT, "tests/integration/docker/compose.yaml")} down`, {
        stdio: "pipe",
      });
    } catch {
      // Ignore errors
  ...
  private needsDocker(): boolean {
    return this.suites.some((s) => s.environment === "docker" || s.environment === "both");
  }
  ...
  private needsQEMU(): boolean {
    return this.suites.some((s) => s.environment === "qemu" || s.environment === "both");
  }
  ...
  private hasFailures(): boolean {
    return Array.from(this.context.results.values()).some((r) => r.status === "failed");
  }
...
export { TestSuite, TestResult, OrchestratorConfig, TestEnvironment };
