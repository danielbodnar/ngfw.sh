...
export const testSuites: TestSuite[] = [
...

  {
    ...
    description: "Verify Docker and BuildKit are available with aarch64 support",
    environment: "docker",
    timeout: 30000,
    retries: 0,
    dependencies: [],
...

  {
    ...
    fixtures: [],
    parallel: false,
    command: "docker compose -f tests/integration/docker/compose.yaml build --no-cache agent",
  },

  {
    ...
    fixtures: [],
    parallel: false,
    command: "cross build -p ngfw-agent --release --target aarch64-unknown-linux-musl",
  },
...

  {
    ...
    description: "Agent successfully authenticates with valid device_id and api_key",
    environment: "docker",
    timeout: 60000,
    retries: 1,
...

  {
    ...
    description: "Agent collects and reports system metrics (CPU, memory, uptime)",
    environment: "docker",
    timeout: 90000,
    retries: 2,
    dependencies: ["agent-connect-docker"],
...

  {
    ...
    description: "Agent correctly detects and reports firmware version from NVRAM",
    environment: "docker",
    timeout: 60000,
    retries: 1,
    dependencies: ["agent-connect-docker"],
...

  {
    ...
    timeout: 120000,
    retries: 1,
    dependencies: ["agent-connect-docker", "auth-valid-credentials", "metrics-collection-docker"],
...

  {
    ...
    description: "Metrics format is consistent across Docker and QEMU environments",
    environment: "both",
    timeout: 180000,
    retries: 1,
 ...
 */
export function getSuitesByEnvironment(env: "docker" | "qemu" | "both"): TestSuite[] {
  return testSuites.filter((suite) => suite.environment === env || suite.environment === "both");
}
